...
export const MONSTER_SUMMONER_FEATS: FeatDefinition[] = [
...

  {
    ...
    prerequisites: [
      { type: 'feat', featId: 'spell_focus_conjuration' },
      {
        type: 'special',
        description:
          'Ability to cast lesser planar ally, lesser planar binding, or a higher-level version of these spells',
      },
    ],
    effects: [
      {
        ...
        target: 'special.planar_binding_charisma_check',
        value: 2,
        source: 'Augment Calling',
        condition: {
          type: 'custom',
          description:
            'When making Charisma checks to request services via planar binding against chosen outsider subtype',
          params: {},
        },
      },
    ],
...

  {
    ...
    description:
      'Upon confirming a critical hit using Arcane Strike or an arcane pool-enhanced weapon against a summoned creature (one you identified via Spellcraft), the target must succeed at a Will save (DC = 10 + 1/2 character level + Intelligence modifier) or be sent back to its home plane as per the dismissal spell.',
    shortDescription:
...

  {
    ...
    description:
      'If a summoned or called creature materializes in a space you threaten, you can make an attack of opportunity against that creature before it acts, gaining a +2 circumstance bonus on the attack roll if you successfully identified the spell used to summon or call that creature.',
    shortDescription:
    ...
    effects: [
      {
        ...
        condition: {
          type: 'custom',
          description:
            'When making an attack of opportunity against a summoned creature and the summoning spell was successfully identified',
          params: {},
        },
      },
    ],
...

  {
    ...
    description:
      'Whenever you attempt a dispel check based on your caster level, you gain a +2 bonus to the check. This stacks with the bonus from Dispel Focus, providing a total of +4 on dispel checks.',
    shortDescription:
...

  {
    ...
    name: 'Solid Shadows',
    description:
      "When casting greater shadow conjuration, greater shadow evocation, shadow conjuration, or shadow evocation (and similar spells with the shadow descriptor at GM discretion), that spell is 20% more real than normal. A solid shadows spell takes up a spell slot 1 level higher than the spell's actual level.",
    shortDescription:
...

  {
    ...
    prerequisites: [
      {
        type: 'special',
        description: "Ability to cast summon monster III or summon nature's ally III",
      },
    ],
    effects: [],
    activationMode: 'conditional',
    ...
    effects: [
      {
        ...
        condition: {
          type: 'custom',
          description:
            'While a summoned creature is stored as a tattoo via Tattoo Attunement; gain one elemental resistance the tattooed creature possesses',
          params: {},
        },
      },
    ],
...

  {
    ...
    description:
      "When casting summon nature's ally spells, you can apply one of five templates (aerial, aqueous, chthonic, fiery, or primordial) to summoned animals, humanoids, or vermin instead of granting Augment Summoning benefits. If you summon multiple creatures with one casting, they must all have the same template.",
    shortDescription:
